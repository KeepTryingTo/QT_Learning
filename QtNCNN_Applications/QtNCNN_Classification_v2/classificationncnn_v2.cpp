...
#include "ui_classificationncnn_v2.h"

ClassificationNCNN_v2::ClassificationNCNN_v2(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::ClassificationNCNN_v2)
{
    ui->setupUi(this);
...
ClassificationNCNN_v2::~ClassificationNCNN_v2()
{
    delete ui;
}

QList<QString> ClassificationNCNN_v2::readClassesFromFile(const QString &filePath) {
...
QImage ClassificationNCNN_v2::cvMatToQImage(const cv::Mat &inMat) {
    switch (inMat.type()) {
    // 8-bit, 4 channel
...
int ClassificationNCNN_v2::detect_net(const cv::Mat& bgr, std::vector<float>& cls_scores){
    ncnn::Net model;
    ...
    if (model.load_param(this -> modelPath.toStdString().c_str()))
        exit(-1);
    if (model.load_model(QString(this -> modelPath.split(".")[0] + ".bin").toStdString().c_str()))
        exit(-1);
...
std::vector<float> ClassificationNCNN_v2::softMax(std::vector<float> cls_scores){
...
int ClassificationNCNN_v2::print_topk(const std::vector<float>& cls_scores, int topk){
...
void ClassificationNCNN_v2::on_btn_open_file_clicked()
{
...
void ClassificationNCNN_v2::on_btn_open_model_clicked()
{
...
void ClassificationNCNN_v2::on_btn_predict_clicked()
{
    //开始预测时间
...
void ClassificationNCNN_v2::on_btn_close_clicked()
{
    this ->  close();
}
