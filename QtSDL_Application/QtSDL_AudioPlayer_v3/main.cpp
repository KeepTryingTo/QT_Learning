...
int main(int argc, char *argv[])
{
    ...
    Mix_Music * music;
    //当点击暂停之后，用于保存上一次播放的位置
    float currPlayPosition = 0;
    ...
    while(running){
        while(SDL_PollEvent(&e)){ //用于从事件队列中获取待处理的事件
            ...
            if(e.type == SDL_KEYDOWN){ //按下键盘响应事件
                ...
                if(isMouseOverButton(open -> rect,x,y)){
                    ...
                    // 获取总时长（秒）
                    Mduration = (qint64)Mix_MusicDuration(music);
                    if (Mduration < 0) {
                        SDL_Log("Failed to get music duration! SDL_mixer Error: %s\n", Mix_GetError());
                    }

                }else  if(isMouseOverButton(pause -> rect,x,y)){
                    //这个功能没有实现，暂时先留着
                    isPaused = !isPaused;
                    if(isPaused){
                        Mix_PauseMusic();
                        currPlayPosition = Mix_GetMusicPosition(music);
                    }else{
                        Mix_PlayMusic(music,0);//不进行循环播放
                        Mix_SetMusicPosition((double)currPlayPosition); //根据上次暂停播放的位置进行设置
                    }
        ...
        if(isMouseOverButton(pause -> rect,x,y)){
        ...
        if(isMouseOverButton(label_time1,x,y)){
            SDL_SetRenderDrawColor(renderer,255,97,0,255);
            SDL_RenderFillRect(renderer,&label_time1);
        }else{
            SDL_SetRenderDrawColor(renderer,192,192,192,255);
            SDL_RenderFillRect(renderer,&label_time1);
        }

        if(isMouseOverButton(label_time2,x,y)){
            SDL_SetRenderDrawColor(renderer,255,97,0,255);
            SDL_RenderFillRect(renderer,&label_time2);
        }else{
            SDL_SetRenderDrawColor(renderer,192,192,192,255);
            SDL_RenderFillRect(renderer,&label_time2);
        }
        ...
        //获得当前播放的位置，并更新显示在屏幕上,刚开始的时候返回得到的为-1，因此我们赋值为0
        if(music)currPlayPosition = Mix_GetMusicPosition(music);
        if(currPlayPosition == -1)currPlayPosition = 0;

        //显示总时长时间
        timeString = updateDuration((qint64)currPlayPosition); //从秒转换为时间格式hh::mm::ss
        durationText = timeString.toLocal8Bit().data();
        durationTexture = loadTextTexture(renderer,font,durationText,textColor);
        //获取当前文本高宽
        durationTextRect = getTextureSize(durationTexture,label_time1);
        SDL_RenderCopy(renderer, durationTexture, NULL, &durationTextRect);
